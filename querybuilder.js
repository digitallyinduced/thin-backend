import { DataSyncController, DataSubscription } from './datasync.js';
...
function whereIn(...args) {
    return (new ConditionBuilder()).whereIn(...args);
}
...
class QueryBuilder extends ConditionBuildable {
    ...
    async fetch() {
        ...
        const { result } = await dataSyncController.sendMessage({
            tag: 'DataSyncQuery',
            query: this.query,
